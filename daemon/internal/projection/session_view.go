...
func EditableUserMessage(ss protocol.Session, events []protocol.AstralEvent, status string) *protocol.EditableUserMessageView {
	...
	for index := len(events) - 1; index >= 0; index-- {
		...
		return &protocol.EditableUserMessageView{EventSeq: event.Seq, Text: text}
	}
	return nil
}
...
func ProjectedSessionTitle(fallback string, events []protocol.AstralEvent) string {
...
func PendingInteraction(events []protocol.AstralEvent) *protocol.PendingInteractionView {
...
func buildPendingInteractionView(event protocol.AstralEvent, value map[string]any, id string) *protocol.PendingInteractionView {
	...
	view := &protocol.PendingInteractionView{
		ID:         id,
		Kind:       kind,
...
func interactionActions(kind string, value map[string]any, params map[string]any) []protocol.InteractionActionView {
	if kind == "plan" {
		label := "批准并执行"
		if stringValue(value["source"]) == "claude" {
			label = "接受计划"
		}
		return []protocol.InteractionActionView{
	...
	actions := []protocol.InteractionActionView{{ID: "accept", Label: acceptLabel, Role: "primary"}}
	if rawKind == "file_change" {
		actions = append(actions, protocol.InteractionActionView{ID: "acceptForSession", Label: "本 session 允许", Role: "secondary"})
	}
	actions = append(actions, protocol.InteractionActionView{ID: "decline", Label: "拒绝", Role: "danger"})
	return actions
}

func explicitInteractionActions(value any) []protocol.InteractionActionView {
	values, ok := value.([]any)
	if !ok {
		return nil
	}
	actions := []protocol.InteractionActionView{}
	for _, item := range values {
		...
		actions = append(actions, protocol.InteractionActionView{ID: decision, Label: decisionLabel(decision), Role: role})
	}
	return actions
}
...
func interactionDetailRows(kind string, value map[string]any, params map[string]any) []protocol.InteractionDetailRow {
	rows := []protocol.InteractionDetailRow{}
	add := func(key, label string, raw any, mono bool) {
		text := stringValue(raw)
		if text == "" {
			return
		}
		rows = append(rows, protocol.InteractionDetailRow{Key: key, Label: label, Value: text, Mono: mono})
	}
	if kind == "plan" {
		if plan := firstNonNil(value["text"], value["plan"]); plan != nil {
			rows = append(rows, protocol.InteractionDetailRow{Key: "plan", Label: "计划", Value: jsonPreviewAny(plan)})
		}
	}
	...
	if permissions := firstNonNil(value["permissions"], params["permissions"], value["additional_permissions"], params["additionalPermissions"]); permissions != nil {
		rows = append(rows, protocol.InteractionDetailRow{Key: "permissions", Label: "权限", Value: jsonPreviewAny(permissions), Mono: true})
	}
	if network := firstNonNil(value["network_approval_context"], params["networkApprovalContext"]); network != nil {
		rows = append(rows, protocol.InteractionDetailRow{Key: "network", Label: "网络", Value: jsonPreviewAny(network), Mono: true})
	}
	if changes := firstNonNil(value["changes"], params["changes"]); changes != nil {
		rows = append(rows, protocol.InteractionDetailRow{Key: "changes", Label: "变更", Value: jsonPreviewAny(changes), Mono: true})
	}
	return rows
}

func QueuedInputs(events []protocol.AstralEvent) []protocol.QueuedInputView {
	pending := map[string]protocol.QueuedInputView{}
	order := []string{}
	for _, event := range events {
		value := mapValue(event.Normalized)
		...
		case "queue.queued":
			...
			pending[id] = protocol.QueuedInputView{ID: id, SessionID: event.SessionID, Text: text}
		case "queue.dequeued", "queue.cancelled", "queue.failed", "queue.rejected", "queue.steered":
			delete(pending, id)
		}
	}
	out := []protocol.QueuedInputView{}
...
func normalizedSessionTitleText(value map[string]any) string {
	text := strings.TrimSpace(stringValue(value["text"]))
	if text == "" {
		return ""
	}
...
func normalizedNativeSessionTitle(value map[string]any) (string, int) {
	for _, key := range []string{"title", "summary", "name"} {
		if text := normalizeSessionTitleString(stringValue(value[key])); text != "" {
			return text, 20
		}
	}
	return "", 0
