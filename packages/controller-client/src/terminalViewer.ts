export type TerminalViewerReadyPayload = {
  terminal_id?: string;
  viewer_id?: string;
...
export type TerminalViewerStatusPayload = {
  terminal_id?: string;
...
export type TerminalViewerHeartbeatPayload = {
  terminal_id?: string;
  viewer_id?: string;
  input_lease_id?: string;
...
export type TerminalViewerStreamHandlers = {
  onOpen?: () => void;
  onReady?: (payload: TerminalViewerReadyPayload) => void;
  onStatus?: (payload: TerminalViewerStatusPayload) => void;
  onHeartbeat?: (payload: TerminalViewerHeartbeatPayload) => void;
...
export type TerminalViewerConnection = {
...
export type TerminalViewerOpenOptions = {
  terminalId?: string;
  afterSeq?: number;
};

export type TerminalViewerTerminalClient = {
...
export type TerminalViewerCoreClient = {
  readonly terminal: TerminalViewerTerminalClient;
};
...
export type TerminalViewerControllerOptions = {
  api: TerminalViewerCoreClient;
  workspaceId: string;
  terminalId: string;
  onOpen?: () => void;
  onReady?: (payload: TerminalViewerReadyPayload) => void;
...
export class TerminalViewerController {
  private connection: TerminalViewerConnection | null = null;
    ...
    this.connection = this.options.api.terminal.openWorkspaceTerminal(
      this.options.workspaceId,
      {
        ...
        onOutput: (data, outputSeq) => {
          ...
          this.options.onOutput(data, markRendered, outputSeq);
        },
        onExit: (payload) => {
  ...
  private applyStatus(payload: TerminalViewerStatusPayload): void {
    if (payload.can_input !== undefined) {
      this.hostAllowsInput = payload.can_input === true;
...
export function isTerminalViewerLifecycleError(message: string, code?: string): boolean {
  ...
  return normalizedMessage === "terminal viewer is not attached"
    || normalizedMessage === "terminal input requires an attached healthy viewer"
    || normalizedMessage === "terminal viewer is not live";
}
