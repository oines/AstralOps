...
`control-client smoke --relay` 强制跳过 LAN/direct transport，只通过 cloud broker 的 opaque relay envelope 做控制通道握手和单次 request/response 验证。它必须提供 `--cloud-base-url`、`--cloud-token` 和本机 known host 中已有的 `--host-device-id`；执行前会用 cloud device registry 校验目标 Host 在线、`can_host=true`，并且云端 public key/fingerprint 与本机 known host 匹配。当前 relay MVP 只覆盖同步 request/response smoke，例如 `core.read.*`、`workspace.files.read/write/apply_patch/move/delete`、`workspace.exec` 和 `host.trust.list`；`--event-subscription`、`--stream-path`、`--attachment-path`、`--media-event-seq/--media-id`、`--terminal` 会被明确拒绝，直到 relay streaming transport 抽象落地。云端和 relay 节点仍然只看到 account/device public metadata、presence/routing metadata、trust/revocation metadata 和 opaque sealed envelopes，不接收 workspace/session/event payload、文件内容、PTY 输出、SSH 配置、附件或媒体内容。
