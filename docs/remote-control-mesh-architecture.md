...
客户端不能把 `cloud.base_url` 直接当 relay URL 使用。它必须从账号响应的 `relay.relay_url` 构造 `RelayClient`。Cloud control plane 和 relay 是部署边界不同的服务：cloud 负责账号、设备 registry、presence、pairing signal、吊销和账号默认 relay 配置；relay 只负责 opaque envelope 投递。开发测试可以临时把二者指向同一个进程或同一个域名，但 daemon 代码必须继续通过 `CloudClient` / `RelayClient` 分离调用。
...
公网 relay 必须配置独立 allowlist，不能依赖 cloud 进程内状态：

```text
ASTRALOPS_RELAY_ACCOUNT_TOKENS=<long-random-token-1>,<long-random-token-2>
```

内测阶段可以让 cloud 和 relay 使用同一组 account token，cloud 只负责在 `GET /v1/account` 返回 `relay_url`；relay 只校验 token 派生出的账号 namespace 并投递 opaque envelope。正式账号体系落地后，优先改成 cloud 下发短期 relay credential，而不是让 relay 读取 cloud 数据库。
