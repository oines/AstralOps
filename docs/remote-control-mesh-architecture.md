...
Desktop Host 永远是执行权威。workspace state、AstralOps control state、Claude/Codex native history、Claude/Codex runtime、SSH key、SSH workspace 操作、PTY 进程、pending interaction、本地文件和凭据都留在 Host 机器上。SSH workspace 的远端机器不运行 Claude/Codex，也不保存 native history；远端 `proxy-agent` 只执行 SSH stdio 上的文件、命令和 PTY 请求。
...
```text
workspace state / AstralOps control state / Claude/Codex native history
