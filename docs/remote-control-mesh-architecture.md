...
`control-client smoke --relay` 强制跳过 LAN/direct transport，只通过 cloud broker 的 opaque relay envelope 做控制通道握手和 E2EE control frame 投递。它必须提供 `--cloud-base-url`、`--cloud-token` 和本机 known host 中已有的 `--host-device-id`；执行前会用 cloud device registry 校验目标 Host 在线、`can_host=true`，并且云端 public key/fingerprint 与本机 known host 匹配。relay transport 复用同一套 request/response 和 stream frame 语义，覆盖 `core.read.*`、`workspace.files.read/write/apply_patch/move/delete/stream`、`workspace.exec`、`core.subscribe.events`、`attachment.ingest.*`、`media.stream`、Host-owned PTY 和 `host.trust.*`。云端和 relay 节点仍然只看到 account/device public metadata、presence/routing metadata、trust/revocation metadata 和 opaque sealed envelopes，不接收 workspace/session/event payload、文件内容、PTY 输出、SSH 配置、附件或媒体内容。
