...
客户端不能把 `cloud.base_url` 直接当 relay URL 使用。它必须从账号响应的 `relay.relay_url` 构造 `RelayClient`。当前开发阶段允许同一个 Go broker 进程同时挂 cloud API 和 relay envelope API，所以 `cloud.base_url` 和 `relay.relay_url` 可以相同；这只是部署复用，不是架构耦合。
...
开发阶段如果 cloud broker 和 relay API 部署在同一个 VPS 进程里，`ASTRALOPS_ACCOUNT_RELAY_URL` 可以填同一个公网 broker URL。后续拆出独立 relay 节点时，只改这个账号 relay URL，不改客户端信任模型、Host trust store 或 E2EE control channel。
...
cloud broker 不存 token 明文，只用 token 派生 `account_id_hash`。同一个 token 对应同一个账号 namespace。这个方案是内测账号基础设施，不是最终消费者账号体验；后续可以把 token auth 替换成正式邮箱登录、OIDC 或 passkey，但外部 API 仍保持账号下设备注册、presence、pairing signal、账号 relay 配置、relay envelope 这几个边界。
...
当前 MVP 中 Cloud broker 进程可以同时提供 Relay API，但 daemon 代码必须通过独立的 `RelayClient` 调用这些 endpoint。
...
daemon 启动后如果 `cloud.enabled=true`，会先读取 `GET /v1/account` 的账号 relay 配置，再向 cloud broker 注册当前设备并定时 heartbeat。注册和 heartbeat 中的 `relay_url` 只是 presence/routing metadata，来自账号默认 relay，不是每台设备任意选择的 relay。关闭 cloud settings 时，daemon 会尝试把当前设备标记为 offline。自动同步只发送 public device identity、capabilities、can_host/can_control、presence 和账号 relay routing metadata，不发送工作区、session、事件、SSH 或路径数据。
...
当前 Desktop MVP 的 Host selector 会展示本机和云端发现的 Desktop Host。`GET /v1/remote/hosts` 必须用 `known_identity` 显式告诉 UI 该 Host 是否已经写入本机 known host，Desktop 不能用 URL 字段猜测信任状态。对于只有 cloud registry 记录、但本机还没有 known host identity 的 Host，主页只能显示“请求控制”入口，不能加载 workspace/session，也不能假装已经可控。点击请求控制会通过本机 daemon 向 cloud broker 创建 pairing request；目标 Host 同步到本地 pending request 后，必须在远控设置页或已有可信 `host.manage` Controller 中批准/拒绝。批准前 Controller 端仍不能进入 Host workspace/session。批准后，Controller daemon 可以把 cloud registry 中目标 Host 的 public identity 写入本机 `known_hosts.json`，使 Host selector 进入可连接状态；这只是 Host identity cache，不是信任授权，后续握手和每个 action 仍必须由目标 Host 本地 trust grant 校验。
...
当前 relay MVP 已落地 Host 侧 envelope polling 和 Controller 侧 encrypted frame transport：当 LAN discovery/direct WebSocket 不可用且目标 Host 是已知可信设备、cloud registry 显示在线时，本机 daemon 可以通过 cloud broker 投递 `control.hello` / `control.hello_ack` / `control.sealed_frame` 完成同一套 E2EE 握手，并在同一条逻辑 control connection 上承载 request/response、event subscription、workspace/media stream、attachment chunk ingest 和 remote PTY frame。
