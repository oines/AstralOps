...
When fixing bugs or implementing behavior, first identify the control plane that should own the decision, lifecycle, state transition, retry, or recovery logic. Do not patch each symptom in separate handlers, UI effects, transport adapters, or daemon endpoints if one control surface should coordinate them. If no clear control plane exists, pause and decide whether to create or consolidate one before writing implementation code.
...
控制面优先原则：每遇到一个错误或需求，先找负责该行为的控制面，例如 Host/Core manager、状态服务、协议层、传输层或 UI renderer 边界。不要先按“这里补一点、那里补一点”的方式让多个模块各管一摊；如果现有代码没有控制面，先判断是否应该收敛出新的控制面或把逻辑迁回已有控制面，再实现具体修复。
