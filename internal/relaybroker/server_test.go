...
func TestRelayBrokerHTTPEnvelopeReachesWebSocketReceiver(t *testing.T) {
	server := newTestRelayServer(t)
	defer server.Close()
	credential := testRelayCredential(t, "acct_test")
	receiver := relayWebSocketDial(t, server.URL, credential, "dev_b")
	...
	if frame.Type != "envelope" || frame.Envelope == nil || frame.Envelope.EnvelopeID != created.EnvelopeID {
		t.Fatalf("frame = %#v, want websocket delivery of HTTP envelope %#v", frame, created.EnvelopeID)
	}
	listed := relayRequest[EnvelopeListResponse](t, http.MethodGet, server.URL+"/v1/relay/envelopes?device_id=dev_b", credential, nil)
	if len(listed.Envelopes) != 0 {
		t.Fatalf("listed = %#v, want empty queue after websocket delivery", listed)
	}
}

func TestRelayBrokerWebSocketEnvelopeReachesHTTPReceiver(t *testing.T) {
	server := newTestRelayServer(t)
	defer server.Close()
	credential := testRelayCredential(t, "acct_test")
	sender := relayWebSocketDial(t, server.URL, credential, "dev_a")
	...
	listed := relayRequest[EnvelopeListResponse](t, http.MethodGet, server.URL+"/v1/relay/envelopes?device_id=dev_b&limit=10", credential, nil)
	if len(listed.Envelopes) != 1 || listed.Envelopes[0].FromDeviceID != "dev_a" || listed.Envelopes[0].ToDeviceID != "dev_b" {
		t.Fatalf("listed = %#v, want websocket envelope queued for HTTP receiver", listed)
	}
}

func TestRelayBrokerQueuedEnvelopeReachesWebSocketOnConnect(t *testing.T) {
	server := newTestRelayServer(t)
	defer server.Close()
	credential := testRelayCredential(t, "acct_test")
	...
	if frame.Type != "envelope" || frame.Envelope == nil || frame.Envelope.EnvelopeID != created.EnvelopeID {
		t.Fatalf("frame = %#v, want queued envelope %#v delivered on websocket connect", frame, created.EnvelopeID)
	}
	listed := relayRequest[EnvelopeListResponse](t, http.MethodGet, server.URL+"/v1/relay/envelopes?device_id=dev_b", credential, nil)
	if len(listed.Envelopes) != 0 {
		t.Fatalf("listed = %#v, want queue drained after websocket connect", listed)
	}
}
